        ...
        <section class="profile-number">
          <router-link :to="userInfo._id ? '/userinfo':'/login'" class="profile-link">
              <span class="profile_image">
                <i class="iconfont icon04"></i>
              </span>
              <div class="user-info">
                <p class="user-info-top" v-if="!userInfo._id">{{userInfo.username ||'登录/注册'}}</p>
                <p>
                <span class="user-icon">
                  <i class="iconfont iconshouji"></i>
                </span>
                  <span class="icon-mobile-number">{{userInfo.tel ||'暂无绑定手机号'}}</span>
                </p>
              </div>
              <span class="arrow">
      ...
      methods:{
        loginOut(){
          MessageBox.confirm('确定退出?').then(action => {
            this.$store.dispatch('loseLoginInfo')
            Toast('已退出')
          },
          action=>{
            console.log('取消了')
